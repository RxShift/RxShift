...
- [ ] **Security follow-ups (Jamison's call, from the June 23 audit):** confirm OpenAI DPA/zero-retention for staff-name+schedule egress; add a Content-Security-Policy (other security headers already shipped); optional CAPTCHA on the demo form; enable Supabase "leaked password protection" (dashboard); audit-log platform-admin emulation/tenant-switch.

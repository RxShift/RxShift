...
//
// Two distinct deficiency stories in the current week:
//  • CEILING (Henderson, Thursday 2–4 PM): Dr. Sunita Patel left at 2:00 PM on
//    a family emergency and the float pharmacist, Dr. Owen Fitzgerald, was held
//    up at Spring Valley until 4:00 PM — so the technicians kept the counter
//    open with no pharmacist on duty.
//  • FLOOR (North Las Vegas, Tuesday 9–10 AM): a tech called out, so Dr. Chang
//    opened solo with no support staff — under Nevada R072-25 a single
//    pharmacist must have at least one technician on duty.
// The engine flags exactly those slots; both are isolated incidents, not a
// sustained deficiency.
//
// R072-25 is enabled for this tenant (tenant.nevada_r072_25 = true): all three
// locations are retail, Spring Valley has a drive-through (floor of 2 support
// staff for a solo pharmacist), and the two "Pharmacy Trainee" techs carry
// staff_type = tech_in_training for the trainee sublimit.
//
...
/** Map a roster person to the R072-25 staff_type. Pharmacists → pharmacist;
 *  "Pharmacy Trainee" titles → tech_in_training (the trainee sublimit); every
 *  other technician → tech. (No non_counting staff in the Mesa Vista roster.) */
function staffTypeFor(p: Person): "pharmacist" | "tech" | "tech_in_training" {
  if (p.ratio === "pharmacist") return "pharmacist";
...
const SV_LINES: ShiftLine[] = [
  ...
  // Saturday: Marcus is the only pharmacist, and Spring Valley has a drive-through
  // (R072-25 floor = 2 support staff for a solo pharmacist), so keep two techs on
  // the whole Saturday shift — the flagship stays compliant even under the floor.
  { staff: "Carlos Rivera", days: [6], start: "09:00", end: "17:00", breakMin: 30 },
  ...
  { staff: "Dana Holt", days: [6], start: "09:00", end: "17:00", breakMin: 30 },
...
export async function seedMesaVista(
  ...
  // Idempotent: a reset preserves the tenant row, so re-assert the R072-25
  ...
  // Locations (ratio is computed per location — no zones). All retail. Spring
  // Valley has a drive-through (so under R072-25 a solo pharmacist there needs
  // TWO support staff — it's staffed to stay compliant). Expected Rx volumes are
  // informational only (shown on the schedule; never enforced).
  type LocSeed = {
    key: "SV" | "HD" | "NLV";
    name: string;
    address: string;
    hasDriveThrough: boolean;
    rx: [number, number, number, number, number, number, number]; // Mon..Sun
  };
  const LOCS: LocSeed[] = [
    {
      key: "SV",
      name: "Mesa Vista — Spring Valley",
      address: "4880 S Rainbow Blvd, Las Vegas, NV",
      hasDriveThrough: true,
      rx: [520, 540, 530, 545, 560, 300, 0],
    },
    {
      key: "HD",
      name: "Mesa Vista — Henderson",
      address: "2310 W Horizon Ridge Pkwy, Henderson, NV",
      hasDriveThrough: false,
      rx: [360, 350, 370, 365, 380, 210, 0],
    },
    {
      key: "NLV",
      name: "Mesa Vista — North Las Vegas",
      address: "3175 W Craig Rd, North Las Vegas, NV",
      hasDriveThrough: false,
      rx: [240, 250, 245, 255, 260, 140, 0],
    },
  ];
  const locIds = new Map<string, string>();
  const locNames = new Map<string, string>();
  const locDriveThrough = new Map<string, boolean>();
  for (const loc of LOCS) {
    const { data: row, error } = await service
      ...
      .insert({
        ...
        has_drive_through: loc.hasDriveThrough,
        expected_rx_mon: loc.rx[0],
        expected_rx_tue: loc.rx[1],
        expected_rx_wed: loc.rx[2],
        expected_rx_thu: loc.rx[3],
    ...
    locDriveThrough.set(loc.key, loc.hasDriveThrough);
  }
  log("3 locations (all retail; Spring Valley has a drive-through).");
  ...
  // A SECOND, different kind of deficiency: Tuesday of the current week at
  // North Las Vegas, where Dr. Chang opened solo with no technician until 10:00
  // — a staffing-FLOOR deficiency under R072-25 (distinct from Henderson's
  // no-pharmacist ceiling gap). Both are current-week so the finalizer records
  // them no matter which weekday the demo is reset on.
  const floorTuesday = addDaysStr(anchor, 1);
    ...
    for (const loc of LOCS) {
      ...
      for (const date of dates) {
        ...
        for (const line of LOC_LINES[loc.key]) {
          ...
          // The North Las Vegas story: Rachel called out that morning, so
          // Dr. Chang opened alone with no tech until 10:00 — the R072-25
          // staffing floor (a solo pharmacist needs ≥1 support) is breached.
          if (
            loc.key === "NLV" &&
            date === floorTuesday &&
            line.staff === "Rachel Odom"
          ) {
            start = "10:00";
          }
      ...
      const engineSegs: EngineSegment[] = shiftInputs.map((s, i) => {
        ...
        return {
          ...
          break_minutes: s.breakMin,
          staff: {
            id: staffIds.get(s.staff)!,
            full_name: s.staff,
            ratio_type: person.ratio,
            is_trainee: staffTypeFor(person) === "tech_in_training",
            certified: person.title.includes("CPhT"),
          },
          work_type: wt
      ...
      // R072-25 is on for this demo tenant — build the location-aware rule so the
      // forecast snapshot matches what the finalized Compliance Record will show.
      const snapshotRule = buildEngineRule(
        { state: "NV", max_techs_per_pharmacist: 3 },
        {
          locationType: "retail",
          hasDriveThrough: locDriveThrough.get(loc.key) ?? false,
          r072Enabled: true,
        }
      );
      const evals = evaluateZone(engineSegs, snapshotRule, 30);
  ...
  log(
    `${deficientHours} deficient compliance hours (expected: 3 — Henderson Thursday 2–4 PM ceiling gap, North Las Vegas Tuesday 9–10 AM floor gap).`
  );
  ...
  await service.from("override_log").insert({
    ...
    warning_type: "ratio",
    reason:
      "Dr. Patel had a family emergency and left at 2:00 PM. Float pharmacist Dr. Fitzgerald was notified immediately but was held at Spring Valley until 4:00 PM. Coverage gap documented. A single isolated incident — not a sustained deficiency.",
  });
    ...
    },
    {
      ...
      body_html: brandedEmailHtml({
        bodyHtml: emailLines([
          ...
          "This was a single isolated incident — not a sustained deficiency.",
        ]),
      }),
      status: "sent",
      actor_user_id: frankAuthId,
  ...
  if (tenantRow) {
    ...
    for (const row of (deficientHrs ?? []) as { id: string }[]) {
      await service.from("compliance_record_note").insert({
        ...
        note: "Dr. Patel had a family emergency and left at 2:00 PM. Float pharmacist Dr. Fitzgerald was notified immediately but was held at Spring Valley until 4:00 PM. Coverage gap documented. A single isolated incident — not a sustained deficiency.",
      });
    }
    log(`Annotated ${(deficientHrs ?? []).length} deficient Henderson hour(s).`);

    // Annotate the North Las Vegas floor gap the same way (a documented,
    // explained understaffing incident — distinct from the Henderson ceiling gap).
    const { data: floorHrs } = await service
      .from("compliance_record")
      .select("id")
      .eq("tenant_id", tenantId)
      .eq("location_id", locIds.get("NLV")!)
      .eq("date", floorTuesday)
      .eq("ratio_status", "deficient");
    for (const row of (floorHrs ?? []) as { id: string }[]) {
      await service.from("compliance_record_note").insert({
        tenant_id: tenantId,
        compliance_record_id: row.id,
        author_user_id: frankAuthId,
        note: "Rachel Odom called out sick that morning. Dr. Chang opened the pharmacy alone with no technician on the floor until 10:00 AM, when Destiny Freeman arrived. Under R072-25 a solo pharmacist needs at least one support person on duty — understaffing documented. Isolated incident.",
      });
    }
    log(`Annotated ${(floorHrs ?? []).length} deficient North Las Vegas floor hour(s).`);
  }
